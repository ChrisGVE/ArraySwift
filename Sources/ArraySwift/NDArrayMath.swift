...
extension NDArray {
  ...
  public func sign() -> NDArray {
    ...
    var result = [Double](repeating: 0, count: size)
    for i in 0..<size {
      if real[i] > 0 { result[i] = 1 } else if real[i] < 0 { result[i] = -1 } else { result[i] = 0 }
    }
    return NDArray(shape: shape, data: result)
  }
  ...
  /// For complex arrays, computes log(z) / log(2).
  public func log2() -> NDArray {
    if isComplex {
      ...
      var resultImag = [Double](repeating: 0, count: size)
      for i in 0..<size {
        resultReal[i] = logZ.real[i] / ln2
        resultImag[i] = logZ.imag![i] / ln2
      }
      return NDArray.complexArray(shape: shape, real: resultReal, imag: resultImag)
    }
  ...
  /// For complex arrays, computes log(z) / log(10).
  public func log10() -> NDArray {
    if isComplex {
      ...
      var resultImag = [Double](repeating: 0, count: size)
      for i in 0..<size {
        resultReal[i] = logZ.real[i] / ln10
        resultImag[i] = logZ.imag![i] / ln10
      }
      return NDArray.complexArray(shape: shape, real: resultReal, imag: resultImag)
    }
  ...
  /// For complex arrays, computes exp(z) - 1.
  public func expm1() -> NDArray {
    if isComplex {
      let expZ = self.exp()
      let one = NDArray.full(shape, real: 1, imag: 0)
      return expZ.subtract(one)
    }
  ...
  /// For complex arrays, computes arcsin(z) = -i * log(iz + sqrt(1 - z²)).
  public func arcsin() -> NDArray {
    if isComplex {
      // arcsin(z) = -i * log(iz + sqrt(1 - z²))
      let one = NDArray.full(shape, real: 1, imag: 0)
      let zSq = self.square()
      let inner = one.subtract(zSq).csqrt()
      // iz: multiply by i means (a+bi) -> (-b + ai)
      let iz = NDArray(
        shape: shape, dtype: .complex128,
        real: imag!.map { -$0 }, imag: real
      )
      let logArg = iz.add(inner)
      let logVal = logArg.clog()
      // -i * (a+bi) = (b - ai)
      return NDArray(
        shape: shape, dtype: .complex128,
        real: logVal.imag!, imag: logVal.real.map { -$0 }
      )
    }
  ...
  /// For complex arrays, computes arccos(z) = -i * log(z + sqrt(z² - 1)).
  public func arccos() -> NDArray {
    if isComplex {
      // arccos(z) = -i * log(z + i*sqrt(1 - z²))
      let one = NDArray.full(shape, real: 1, imag: 0)
      let zSq = self.square()
      let sqrtInner = one.subtract(zSq).csqrt()
      // i * sqrt(...): multiply by i
      let iSqrt = NDArray(
        shape: shape, dtype: .complex128,
        real: sqrtInner.imag!.map { -$0 }, imag: sqrtInner.real
      )
      let logArg = self.add(iSqrt)
      let logVal = logArg.clog()
      // -i * (a+bi) = (b - ai)
      return NDArray(
        shape: shape, dtype: .complex128,
  ...
  /// For complex arrays, computes arctan(z) = i/2 * log((1 - iz) / (1 + iz)).
  public func arctan() -> NDArray {
    if isComplex {
      ...
      let num = one.subtract(iz)
      let den = one.add(iz)
      let ratio = num.divide(den)
      let logVal = ratio.clog()
      // i/2 * (a+bi) = (-b/2 + a/2 * i)
      return NDArray(
        shape: shape, dtype: .complex128,
        real: logVal.imag!.map { -$0 / 2.0 },
        imag: logVal.real.map { $0 / 2.0 }
      )
    }
